...
#Keep one crop row per district-year (as in Regression/q1a.r)
...
ggplot(df_b_i_kharif, aes(x=index_kharif, y=v42_kharif)) + geom_point() + stat_smooth(method = "lm", col="blue")

df_b_i_rabi<- data.frame(index_rabi,v42_rabi)
ggplot(df_b_i_rabi, aes(x=index_rabi, y=v42_rabi)) + geom_point() + stat_smooth(method = "lm", col="blue")
