...
def main() -> None:
    ...
    for v in [f"v{i}" for i in range(1, 48)]:
        kind = "count" if p[v].max() > 100 else "percentage or ratio"
        desc[v] = V_DOCUMENTED.get(v, f"Undocumented in the source material (HMIS-style indicator); {kind}, inferred from its range")
    ...
    lines = ["# Codebook for `data/processed/panel.csv`", "",
             ...
             "the rest are carried unchanged and marked undocumented, with count vs percentage inferred from the column range.", "",
